...
import { importDevices, type HasilImport } from "./actions";

export function ImportDevicePanel() {
  ...
  async function handleUpload() {
    ...
    fd.append("file", file);
    const result = await importDevices(fd);
    setHasil(result);
    setLoading(false);
    setFile(null);
      ...
      <div className="space-y-4">
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <p className="mb-4 text-sm text-slate-500">
            Unduh template Excel, isi data perangkat pada sheet{" "}
            <strong>Data</strong>, lalu hapus/timpa 2 baris contoh sebelum
            diunggah.
          </p>
          <a
            href="/templates/template-import-device.xlsx"
            download
            className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-emerald-700"
          >
            <Download className="h-4 w-4" />
            Unduh Template Devices
          </a>
        </div>
        ...
        <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-5 text-sm text-indigo-800">
          ...
          <ul className="list-inside list-disc space-y-1">
            <li>
              <strong>Nama Perangkat</strong> — wajib diisi
            </li>
            <li>Jenis, Status — pilih dari dropdown yang tersedia</li>
            <li>Tanggal Beli — format YYYY-MM-DD</li>
            <li>Harga Beli — angka saja, tanpa Rp/titik</li>
            <li>Perusahaan, Cabang — harus sama persis dengan data terdaftar</li>
            <li>Email Pengguna — harus user yang sudah terdaftar</li>
            <li>Spesifikasi 1–3 — pasangan Nama/Nilai bebas (opsional)</li>
          </ul>
          <p className="mt-2 text-xs text-indigo-600">
            Kode inventaris dibuat otomatis, tidak perlu diisi manual.
          </p>
        </div>
      </div>
